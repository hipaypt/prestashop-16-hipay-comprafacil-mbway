...
class HipayMbway extends PaymentModule {
    ...
    public function install() {
        ...
        if (extension_loaded('soap') == false) {
            ...
            return false;
        } elseif (!parent::install() || !$this->registerHook('payment') || !$this->registerHook('paymentReturn') || !$this->registerHook('displayAdminOrder') || !$this->registerHook('displayOrderDetail')) {
            return false;
        }
    ...
    private function _displayHipayMbway() {
    ...
    public function hookPayment($params) {
        ...
        $this->smarty->assign(array(
            ...
            'this_path_hipaymbway' => $this->_path,
    ...
    public function hookPaymentReturn($params) {
        ...
        $mbwayData = $this->getMbwayData($params['objOrder']->id);
        $state = $params['objOrder']->getCurrentState();
        if (in_array($state, array(Configuration::get('HIPAY_MBWAY_WAITING'), Configuration::get('PS_OS_OUTOFSTOCK'), Configuration::get('PS_OS_OUTOFSTOCK_UNPAID'))) && isset($mbwayData['reference'])) {
            $this->smarty->assign(array(
                'this_path' => $this->_path,
                'this_path_hipaymbway' => $this->_path,
                'this_path_ssl' => Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ . 'modules/' . $this->name . '/',
                ...
                'mbway_reference' => $mbwayData['reference']
            ));
    ...
    private function getMbwayData($order_id) {

        $sql = 'SELECT reference,phone,status,hipay_status,notification_date FROM ' . _DB_PREFIX_ . 'hipaymbway WHERE order_id = ' . $order_id;
    ...
    public function hookDisplayOrderDetail($params) {
        ...
        if ($row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($sql)) {
            ...
            return $this->display(__FILE__, 'views/templates/front/detail_order.tpl');
        }
        return;
    }
